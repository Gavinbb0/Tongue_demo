...
type ReportName = 'single' | 'monthly' | 'visit' | 'hospital' | 'privacy' | 'help' | null;
...
export default function Page() {
  ...
  return (
    <main className="stage">
        ...
        <Tabs value={tab} onValueChange={(value) => setTab(String(value))} className="app-shell">
          <div className="screen">
            ...
            <TabsContent value="records" className="page">
              ...
              <div className="timeline">{entries.map((entry) => <button key={entry.id} onClick={() => entry.kind === 'Tongue' && setReportName('single')}><span className={`timeline-icon ${entry.kind.toLowerCase()}`}>{entry.kind === 'Tongue' ? <FileImage size={19} /> : entry.kind === 'Metrics' ? <FlaskConical size={19} /> : <ClipboardCheck size={19} />}</span><span><small>{entry.date}</small><strong>{entry.title}</strong><em>{entry.detail}</em></span><ChevronRight size={16} /></button>)}</div>
      ...
      <Dialog open={!!reportName} onOpenChange={(open) => !open && setReportName(null)}><DialogContent className="report-dialog"><DialogTitle>{reportName === 'single' ? 'Single tongue analysis' : reportName === 'monthly' ? 'September health trend' : reportName === 'visit' ? (isVip ? 'Visit preparation' : 'VIP monthly report') : reportName === 'hospital' ? 'Hospital care plan' : reportName === 'privacy' ? 'Privacy and data' : 'Help'}</DialogTitle><DialogDescription>{reportName === 'single' ? 'Sep 10 · Patient report' : 'TongueCare patient experience'}</DialogDescription>
