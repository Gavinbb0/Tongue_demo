        ...
        <Tabs value={tab} onValueChange={(value) => setTab(String(value))} className="app-shell">
          <div className="screen">
            ...
            <TabsContent value="reports" className="page">
              ...
              <button className={isVip ? 'report-card' : 'report-card locked'} onClick={() => setReportName('monthly')}><div className="report-card-top"><span className="report-symbol"><TrendingUp size={21} /></span>{!isVip && <LockKeyhole size={17} />}</div><strong>30天舌象变化趋势</strong><p>连续对比舌色、舌苔、舌体形态和其他可见特征。</p><div className="mini-chart" aria-label="舌象变化趋势示意"><i style={{ height: '36%' }} /><i style={{ height: '52%' }} /><i style={{ height: '44%' }} /><i style={{ height: '68%' }} /><i style={{ height: '62%' }} /><i style={{ height: '74%' }} /></div><div className="report-foot"><span>{isVip ? '查看趋势报告' : '预览高级版趋势'}</span><ChevronRight size={17} /></div></button>
              <button className="report-card visit-prep-card" onClick={() => setReportName('visit')}><div className="report-card-top"><span className="report-symbol"><ClipboardCheck size={21} /></span><span className={isVip ? 'report-tag gold' : 'report-tag neutral'}>{isVip ? '完整个性化' : '基础摘要'}</span></div><strong>智能复诊准备</strong><p>把近期舌象变化和辅助记录整理成复诊前可快速查看的摘要。</p><div className="report-foot"><span>打开复诊准备</span><ChevronRight size={17} /></div></button>
