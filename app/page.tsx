...
import {
  ...
  Keyboard, LockKeyhole, Moon, Pill, ScanFace, ShieldCheck, Signal, Sparkles, Stethoscope,
...
type ReportName = 'record' | 'single' | 'monthly' | 'visit' | 'clinician' | 'doctor' | 'hospital' | 'privacy' | 'help' | null;
...
const comparisonRecords: ComparisonRecord[] = [
  { id: '12-jul', date: '12 Jul 2026', time: '08:12', label: 'Baseline', quality: 'Passed', colour: 'Pale pink', coating: '24%', central: 'Mild', moisture: 'Balanced', edges: 'No notable change', fatigue: 'None reported', medication: 'Taken as planned', alt: '42 U/L', ast: '31 U/L', plannedTest: 'Not due' },
  { id: '20-jul', date: '20 Jul 2026', time: '08:06', label: 'Week 2', quality: 'Passed', colour: 'Pale pink', coating: '31%', central: 'Mild–moderate', moisture: 'Balanced', edges: 'No notable change', fatigue: 'Mild, one day', medication: '1 missed dose', alt: '—', ast: '—', plannedTest: 'Scheduled' },
  { id: '28-jul', date: '28 Jul 2026', time: '07:54', label: 'Week 3', quality: 'Passed', colour: 'Pink', coating: '36%', central: 'Moderate', moisture: 'Slightly dry', edges: 'Stable', fatigue: 'Occasional', medication: '1 missed dose', alt: '51 U/L', ast: '34 U/L', plannedTest: 'Scheduled' },
  { id: '4-aug', date: '4 Aug 2026', time: '08:19', label: 'Week 4', quality: 'Passed', colour: 'Pale pink', coating: '39%', central: 'Moderate', moisture: 'Balanced', edges: 'Stable', fatigue: 'More frequent', medication: '2 missed doses', alt: '—', ast: '—', plannedTest: 'Awaiting result' },
  { id: '10-aug', date: '10 Aug 2026', time: '08:28', label: 'Latest', quality: 'Passed', colour: 'Pale pink', coating: '41%', central: 'Moderate–marked', moisture: 'Balanced', edges: 'Stable', fatigue: 'Increased for 2 weeks', medication: '3 missed doses', alt: '58 U/L', ast: '36 U/L', plannedTest: 'Not completed' },
];
...
export default function Page() {
  ...
  const [compareFromId, setCompareFromId] = useState('12-jul');
  const [compareToId, setCompareToId] = useState('10-aug');
  const isVip = plan === 'vip';
  const compareFrom = comparisonRecords.find((record) => record.id === compareFromId) ?? comparisonRecords[0];
  const compareTo = comparisonRecords.find((record) => record.id === compareToId) ?? comparisonRecords.at(-1)!;
  ...
  return (
    <main className="stage">
          ...
          <div className="screen">
            ...
            <TabsContent value="profile" className="page">
              ...
              <div className="menu-list">
                ...
                <div><Bell size={19} /><span><strong>Daily reminder</strong><small>Reminder to complete today’s tongue record</small></span><Switch className="daily-reminder-switch" aria-label="Daily reminder" checked={reminder} onCheckedChange={setReminder} /></div>
      ...
      <Dialog open={!!reportName} onOpenChange={(open) => !open && setReportName(null)}><DialogContent className={`report-dialog ${reportName === 'monthly' && isVip ? 'comparison-dialog' : ''}`}><DialogTitle>{reportName === 'record' ? selectedEntry?.title ?? 'Record details' : reportName === 'single' ? 'Single tongue record summary' : reportName === 'monthly' ? '30-day tongue-image trend' : reportName === 'visit' ? 'Pre-visit information' : reportName === 'clinician' ? 'Clinician summary' : reportName === 'doctor' ? 'Post-visit report' : reportName === 'hospital' ? 'Hospital care programme' : reportName === 'privacy' ? 'Privacy and data' : 'Help'}</DialogTitle><DialogDescription>{reportName === 'record' ? selectedEntry?.date ?? 'Health record' : reportName === 'single' ? '10 August · Ms Li' : reportName === 'monthly' && isVip ? 'Premium · Compare any two accepted records' : reportName === 'visit' ? 'Edit the recent situation, main concerns, and what you want to tell your clinician' : reportName === 'clinician' ? 'Ms Li · Patient-confirmed' : reportName === 'doctor' ? 'Issued by Dr. Wei after the 18 August consultation' : 'TongueCare patient experience'}</DialogDescription>
        ...
        : reportName === 'monthly' ? <div className="dialog-scroll">{!isVip ? <><div className="monthly-score basic"><span><small>Records in 30 days</small><strong>8</strong><em>records</em></span><TrendingUp size={30} /></div><div className="trend-block"><div><span>Coating coverage trend</span><strong>Increased from baseline</strong></div><div className="trend-line"><i /><i /><i /><i /><i /><i /><i /></div><p>Recent photos show increased coating coverage, mainly in the central tongue region.</p></div><div className="basic-trend-summary"><span><CheckCircle2 size={17} /><p><strong>Tongue-body colour</strong>No persistent change</p></span><span><CheckCircle2 size={17} /><p><strong>Moisture-related appearance</strong>No persistent change</p></span></div><div className="vip-callout" role="note"><Crown size={18} /><span><strong>Premium adds detailed review</strong><small>Combines follow-up tongue images, patient reports, and clinical data.</small></span></div><p className="disclaimer">This concise Basic trend report reviews recorded changes and is not for diagnosis.</p></> : <><div className="monthly-score"><span><small>Recording completion</small><strong>25 / 30</strong><em>days</em></span><TrendingUp size={30} /></div><div className="trend-block"><div><span>Coating coverage trend</span><strong>Persistently increased</strong></div><div className="trend-line"><i /><i /><i /><i /><i /><i /><i /></div><p>Coverage remained increased from baseline and was more apparent in the central tongue region.</p></div><section className="comparison-section"><div className="comparison-heading"><div><span><Crown size={15} />Premium comparison</span><h3>Compare individual records</h3></div><small>5 accepted records</small></div><div className="comparison-pickers"><label><span>From</span><select value={compareFromId} onChange={(event) => setCompareFromId(event.target.value)}>{comparisonRecords.map((record) => <option key={record.id} value={record.id}>{record.date} · {record.label}</option>)}</select></label><ChevronRight size={19} /><label><span>To</span><select value={compareToId} onChange={(event) => setCompareToId(event.target.value)}>{comparisonRecords.map((record) => <option key={record.id} value={record.id}>{record.date} · {record.label}</option>)}</select></label></div><div className="comparison-snapshots"><article><span className="snapshot-icon"><ScanFace size={22} /></span><div><small>{compareFrom.label} · {compareFrom.time}</small><strong>{compareFrom.date}</strong><em><CheckCircle2 size={13} />Quality {compareFrom.quality.toLowerCase()}</em></div></article><article><span className="snapshot-icon"><ScanFace size={22} /></span><div><small>{compareTo.label} · {compareTo.time}</small><strong>{compareTo.date}</strong><em><CheckCircle2 size={13} />Quality {compareTo.quality.toLowerCase()}</em></div></article></div><div className="comparison-table"><div className="comparison-table-head"><span>Collected information</span><span>{compareFrom.label}</span><span>{compareTo.label}</span></div>{[
          ['Tongue-body colour', compareFrom.colour, compareTo.colour],
          ['Coating coverage', compareFrom.coating, compareTo.coating],
          ['Central distribution', compareFrom.central, compareTo.central],
          ['Moisture appearance', compareFrom.moisture, compareTo.moisture],
          ['Fissures & edges', compareFrom.edges, compareTo.edges],
        ].map(([label, from, to]) => <div key={label}><strong>{label}</strong><span>{from}</span><span className={from === to ? 'stable' : 'changed'}>{to}</span></div>)}</div><div className="supporting-comparison"><h4>Supporting records</h4>{[
          ['Fatigue', compareFrom.fatigue, compareTo.fatigue],
          ['Medication', compareFrom.medication, compareTo.medication],
          ['ALT', compareFrom.alt, compareTo.alt],
          ['AST', compareFrom.ast, compareTo.ast],
          ['Planned test', compareFrom.plannedTest, compareTo.plannedTest],
        ].map(([label, from, to]) => <div key={label}><strong>{label}</strong><span>{from}</span><ChevronRight size={14} /><span>{to}</span></div>)}</div><div className="comparison-insight"><Sparkles size={18} /><span><strong>What changed between these records</strong><p>{compareFrom.id === compareTo.id ? 'You selected the same record twice. Choose another date to review changes.' : `Coating coverage changed from ${compareFrom.coating} to ${compareTo.coating}. Central coverage changed from ${compareFrom.central.toLowerCase()} to ${compareTo.central.toLowerCase()}. Reported fatigue and medication information are shown alongside the image findings for context.`}</p></span></div></section><p className="disclaimer">This comparison organizes recorded differences. It does not determine their cause, diagnose disease, or recommend treatment.</p></>}</div>
        ...
        : reportName === 'doctor' ? <div className="dialog-scroll doctor-report-detail"><section className="doctor-report-banner"><span><Stethoscope size={25} /></span><div><small>Hospital follow-up · Fictional demo</small><strong>Dr. Wei</strong><p>Hepatology · Issued 18 Aug 2026, 16:20</p></div><em><CheckCircle2 size={14} />Signed</em></section><section className="doctor-report-meta"><span><small>Patient</small><strong>Ms Li · MASLD-10482</strong></span><span><small>Consultation</small><strong>18 Aug 2026</strong></span><span><small>Visit type</small><strong>Routine follow-up</strong></span><span><small>Next review</small><strong>15 Sep 2026</strong></span></section><section className="doctor-note-section"><h3>Consultation summary</h3><p>We reviewed the recent tongue-photo record, patient-reported fatigue, medication adherence, and available hospital test results together. Tongue-image findings remain descriptive and were not used on their own to make a diagnosis.</p></section><section className="doctor-note-section"><h3>Agreed follow-up plan</h3><ul><li>Complete the planned liver-function test before the next scheduled review.</li><li>Continue medicines as prescribed and discuss any further missed doses with the care team.</li><li>Record any continuing fatigue in the daily health record for discussion at follow-up.</li></ul></section><section className="doctor-note-section"><h3>Information reviewed</h3><div className="doctor-reviewed-items"><span><CheckCircle2 size={15} />Tongue-photo history</span><span><CheckCircle2 size={15} />Patient-reported symptoms</span><span><CheckCircle2 size={15} />Medication record</span><span><CheckCircle2 size={15} />Recent liver markers</span></div></section><div className="doctor-next-visit"><CalendarDays size={19} /><span><small>Next appointment</small><strong>15 Sep 2026 · Hepatology clinic</strong></span></div><p className="disclaimer">This is a fictional demonstration report and is not medical advice. Patients should follow instructions provided directly by their own care team.</p></div>
