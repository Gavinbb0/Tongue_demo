  ...
  return (
    <main className="stage">
      ...
      <Dialog open={!!reportName} onOpenChange={(open) => !open && setReportName(null)}><DialogContent className="report-dialog"><DialogTitle>{reportName === 'record' ? selectedEntry?.title ?? '记录详情' : reportName === 'single' ? '单次舌象记录摘要' : reportName === 'monthly' ? '9月舌象变化趋势' : reportName === 'visit' ? '复诊前情况整理' : reportName === 'hospital' ? '医院关怀计划' : reportName === 'privacy' ? '隐私与数据' : '使用帮助'}</DialogTitle><DialogDescription>{reportName === 'record' ? selectedEntry?.date ?? '健康记录' : reportName === 'single' ? '9月10日 · 患者报告' : reportName === 'visit' ? '编辑近期情况、主要担忧和想向医生说明的内容' : '舌康患者端体验'}</DialogDescription>
        ...
        : reportName === 'visit' ? <div className="dialog-scroll visit-prep"><div className={isVip ? 'visit-mode premium' : 'visit-mode'}><span>{isVip ? '高级版 · 深度整理' : '基础版 · 简要概括'}</span><small>{isVip ? '整合30天舌象AI结果、生活记录与体检指标' : '概括近7天舌象AI结果与最近健康记录'}</small></div><div className="visit-edit-form"><div className="visit-summary-editor"><div className="visit-field-title"><label htmlFor="recent-summary">近期情况</label><button type="button" onClick={() => { setVisitSaved(false); setVisitPrep({ ...visitPrep, recent: generateVisitSummary() }); }}><Sparkles size={14} />重新生成</button></div><small className="summary-source">由舌象照片AI整理与健康记录自动生成，可继续编辑</small><textarea id="recent-summary" value={visitPrep.recent} onChange={(event) => { setVisitSaved(false); setVisitPrep({ ...visitPrep, recent: event.target.value }); }} /></div><label><span>主要担忧</span><textarea value={visitPrep.concern} onChange={(event) => { setVisitSaved(false); setVisitPrep({ ...visitPrep, concern: event.target.value }); }} /></label><label><span>想向医生说明的内容</span><textarea value={visitPrep.doctorNote} onChange={(event) => { setVisitSaved(false); setVisitPrep({ ...visitPrep, doctorNote: event.target.value }); }} /></label></div><button className="primary" onClick={() => setVisitSaved(true)}>{visitSaved ? '已保存本次整理' : '保存复诊前情况'}<Check size={18} /></button><p className="disclaimer">{medicalDisclaimer}</p></div>
